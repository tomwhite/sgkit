...
import pandas as pd
from genee_dask import genee_ols
...
def test_simulated_data():
    ...
    # turn ld into a COO dataframe
    ld = ld.to_numpy()
    ld = coo_matrix(ld)
    ld = pd.DataFrame(dict(i=ld.row, j=ld.col, val=ld.data))
    ld = dd.from_pandas(ld, chunksize=chunksize)

    # turn gene list into a dataframe
    ...
    variant_groups = dd.from_pandas(variant_groups, chunksize=chunksize)

    # turn sumstats into a dataframe
    sumstats = pd.DataFrame(dict(beta=mydata.V4.to_numpy()))
    sumstats = dd.from_pandas(sumstats, chunksize=chunksize)

    df = genee_ols(sumstats, ld, variant_groups, gene_start, gene_stop)
    df = df.compute().sort_index()  # need to sort index to compare with expected
