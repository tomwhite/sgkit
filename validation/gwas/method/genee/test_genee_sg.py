...
import pandas as pd
import pytest
from genee_sg import genee_ols_sg, to_sgkit, genee_EM, genee_test2
...
def test_simulated_data_using_join():
    ...
    df = ds.to_dask_dataframe(dim_order=["variants", "samples"])
    ...
    g = [list(zip(sublist, [i] * len(sublist), [len(sublist)] * len(sublist))) for i, sublist in enumerate(g)]
    g = [item for sublist in g for item in sublist] # flatten
    # print(g)

    df = pd.DataFrame(g, columns=["variant_id", "variant_group", "variant_group_length"])
    print(df)

    df = pd.merge(ld, df, left_on="i", right_index=True)
    print(df)
    ...
    def genee_test_group(grp):
        ...
        ld = ld.toarray()
        beta = grp["beta"].to_numpy().reshape(shape)[:, 0] # lots of values are repeated, so throw them away

        print("ld", ld, ld.shape)
        print("beta", beta, beta.shape)
        print("ee", epsilon_effect)

        test_q, q_var, pval = genee_test2(ld, beta, epsilon_effect)
        print(test_q, q_var, pval)

        return pd.DataFrame([[test_q, q_var, pval]], columns=["test_q", "q_var", "pval"])

    grouped = df.groupby("variant_group")
    print(grouped)

    res = grouped.apply(genee_test_group)
    print(res)
